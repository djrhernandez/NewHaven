...
EnemyFighter_Tank::EnemyFighter_Tank(EnemyType et, float p[3], float randFact)
...
void EnemyFighter_Tank::init()
{
	EnemyFighter::init();
}

void EnemyFighter_Tank::init(float *p, float randFact)
{
...
void EnemyFighter_Tank::move()
{
	...
	if (pos[0] < 0.0)
		pos[0] = (game->speedAdj *
				(0.998 * pos[0] + 0.002 * (-config->screenBoundX() + 2.85)));
	else
		pos[0] = (game->speedAdj *
				(0.998 * pos[0] + 0.002 * (config->screenBoundX() - 2.85)));
	switch((age/50) % 8)
