...
#include "EnemyFighter_Gnat.h"
#include "EnemyFighter_RayGun.h"
#include "EnemyFighter_Tank.h"
#include "EnemyFighter_Boss00.h"
#include "EnemyFighter_Boss01.h"

int EnemyFighter::allocated = 0;

EnemyFighter::EnemyFighter(EnemyType et, float p[3], float randFact)
		: itemScreen(itemScreen::ItemEnemy)
{
	type = et;
...
//This function in only here to get rid of the IRIX compiler warning..
void EnemyFighter::init()
{
	itemScreen::init();
}

void EnemyFighter::init(float *p, float randFact)
{
	itemScreen::init();
	over = 0;				//If this points to another aircraft, this AC
							//will be inserted after that AC in the fleet list
	pos[0] = p[0];
	pos[1] = p[1];
	pos[2] = p[2];
...
//Debugging (Shows number of ACs currently instantiated)
void EnemyFighter::printNumAllocated()
{
	fprintf(stderr, "Enemy Aircrafts Allocated: %d\n", allocated);
}

EnemyFighter *EnemyFighter::makeNewEnemy(EnemyType et, float *p, float randFact)
{
	EnemyFighter *enemy = 0;
	switch(et)
	{
		case EnemyStraight:
			enemy = new EnemyFighter_Straight(et, p, randFact);
			break;
		case EnemyOmni:
			enemy = new EnemyFighter_Omni(et, p, randFact);
			break;
		case EnemyRayGun:
			enemy = new EnemyFighter_RayGun(et, p, randFact);
			break;
		case EnemyTank:
			enemy = new EnemyFighter_Tank(et, p, randFact);
			break;
		case EnemyGnat:
			enemy = new EnemyFighter_Gnat(et, p, randFact);
			break;
		case EnemyBoss00:
			enemy = new EnemyFighter_Boss00(et, p, randFact);
			break;
		case EnemyBoss01:
			enemy = new EnemyFighter_Boss01(et, p, randFact);
			break;
		case NumEnemyTypes:
		default:
			enemy = 0;
...
bool EnemyFighter::checkHit(ActiveAmmo *ammo)
{
...
void EnemyFighter::calcShootInterval()
{
	shootInterval = 1;
}
